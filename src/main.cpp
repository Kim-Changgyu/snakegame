...
int main(int argc, char *argv[])
{
  ...
  WINDOW *win1 = newwin(HEIGHT-2, WIDTH-3, 1, 2); // 설정한 가로, 세로 크기에 따라 게임화면 윈도우를 생성
  WINDOW *win2 = newwin(9, 24, 2, WIDTH+2);      // 점수 표시를 위한 고정된 크기의 윈도우 생성
  WINDOW *win3 = newwin(9, 24, 13, WIDTH+2);     // 미션 위한 고정된 크기의 윈도우 생성
